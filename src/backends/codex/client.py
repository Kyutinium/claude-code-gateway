...
class CodexCLI:
    ...
    def resolve(self, model: str) -> Optional[ResolvedModel]:
        ...
        Handles:
        - Exact match: "codex" → backend="codex", provider_model=CODEX_DEFAULT_MODEL
        - ``codex/<sub>`` slash syntax (e.g. "codex/o3")
    ...
    def get_auth_provider(self):
        """Return a CodexAuthProvider instance."""
            ...
            async with self._spawn_codex(
                ...
                async for event in self._parse_jsonl(proc.stdout, line_timeout=self.timeout):
                    ...
                    if event.get("type") == "collab_tool_call":
                        blocks = _collab_to_tool_blocks([event], agent_tool_ids=collab_agent_ids)
                        if blocks:
                            yield {"type": "assistant", "content": blocks}
                        continue

                    normalized = normalize_codex_event(event, agent_tool_ids=collab_agent_ids)
